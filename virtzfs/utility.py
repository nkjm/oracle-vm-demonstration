    ...
    def get_new_name(self, question, choice_list, input, default):
        ...
        while True:
            if name in choice_list:
                print "Specified %s already exist in ZFS repository." % question
                sys.exit(1)
            elif not self.is_valid_name(name):
                print "Specified %s not valid. Valid letters are [a-zA-Z0-9._]" % question
                sys.exit(1)
            else:
                break
            try:
    ...
    def get_name(self, question, choice_list, input, default):
        if input is None:
            ...
            while not name in choice_list:
                print "Specified %s does not exist in ZFS repository." % question
                sys.exit(1)
        return(name)
